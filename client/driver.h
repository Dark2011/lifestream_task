...
public:
	...
	client_driver(boost::asio::io_context& service, const std::string& ipaddr, uint16_t port);	
	~client_driver();

public:
	void initialaze(const boost::filesystem::path& path, int max_pack_size, int random_repeat_pack_precentege);
	void run(mode);
